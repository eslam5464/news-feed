...
class Like:
    ...
    def create(self, like_in: schemas.LikeCreate) -> int:
        cursor = self.conn.cursor()

        query = (
            f"INSERT INTO likes (post_id, user_id, creation_timestamp) "
            f"VALUES ('{like_in.post_id}', '{like_in.user_id}', "
            f"'{like_in.creation_timestamp}')"
        )
    ...
    def get_one(self, like_id: int) -> schemas.Like | None:
        ...
        query = f"SELECT * FROM likes WHERE id = {like_id}"
        cursor.execute(query)
    ...
    def get_all_by_post_id(self, post_id: int):
        ...
        query = f"SELECT * FROM likes WHERE post_id = {post_id}"
        cursor.execute(query)
    ...
    def get_all_by_user_id(self, user_id: int):
        ...
        query = f"SELECT * FROM likes WHERE user_id = {user_id}"
        cursor.execute(query)
        likes_db = cursor.fetchall()
